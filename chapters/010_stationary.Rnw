...
\begin{problem}
Может ли автокорреляционная функция иметь вид $\rho_1 = 0.9$, $\rho_2 = -0.9$, $\rho_k = 0$ при $k\geq 3$?

Может ли частная автокорреляционная функция иметь вид $\phi_{11} = 0.9$, $\phi_{22} = -0.9$, $\phi_{kk} = 0$ при $k\geq 3$?
\begin{sol}
  Автокорреляционная — нет, так как определитель корреляционной матрицы три на три выходит отрицательным. Частная автокорреляционная — да, так как она может быть произвольной, \url{http://projecteuclid.org/euclid.aos/1176342881}.
\end{sol}
\end{problem}
