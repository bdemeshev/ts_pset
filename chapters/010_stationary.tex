...
Вместо чисел $x_0$, $x_1$, \ldots, $x_{N-1}$ мы получаем числа $X_0$, $X_1$, \ldots, $X_{N-1}$.
...
Прямое преобразование Фурье формулой\footnote{Иногда множитель $1/N$ относят к обратному преобразованию Фурье, иногда поровну разносят как $1/\sqrt{N}$.}:
\[
X_k = \frac{1}{N} \sum_{n=0}^{N-1} x_n \exp\left(\frac{-2i\pi}{N} kn \right)
\]
...
\[
x_n = \sum_{k=0}^{N-1} X_k \exp\left(\frac{2i\pi}{N} nk \right)
\]
...
\begin{problem}
Найдите прямое преобразование Фурье последовательности $1$, $4$, $1$, $4$, $1$, $4$.
  \begin{sol}
  \end{sol}
\end{problem}
